...
void main(){
  ...
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;++i){
      ...
      float waveR=speed*t; float ring=exp(-pow((r-waveR)/thickness,2.));
    ...
    if (mustReinit) {
      ...
      if (noiseAmount > 0) {
        ...
        const noiseEffect = new Effect(
          'NoiseEffect',
          `uniform float uTime; uniform float uAmount; float hash(vec2 p){ return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);} void mainUv(inout vec2 uv){} void mainImage(const in vec4 inputColor,const in vec2 uv,out vec4 outputColor){ float n=hash(floor(uv*vec2(1920.,1080.))+floor(uTime*60.)); float g=(n-0.5)*uAmount; outputColor=inputColor+vec4(vec3(g),0.);}`,
      ...
      const animate = () => {
        ...
        if (liquidEffect) {
          const lu = (liquidEffect as Effect & { uniforms: Map<string, THREE.Uniform> }).uniforms;
          const tu = lu.get('uTime');
      ...
      if (t.liquidEffect) {
        const lu = (t.liquidEffect as Effect & { uniforms: Map<string, THREE.Uniform> }).uniforms;
        const us = lu.get('uStrength'); if (us) us.value = liquidStrength;
        const uf = lu.get('uFreq'); if (uf) uf.value = liquidWobbleSpeed;
      }
