...
export default function TargetCursor({
  ...
  useEffect(() => {
    ...
    const scrollHandler = () => {
      ...
      const isStillOverTarget = elementUnderMouse && (elementUnderMouse === activeTarget || (elementUnderMouse as HTMLElement).closest(targetSelector) === activeTarget);
  ...
  return (
    <div ref={cursorRef} className="fixed top-0 left-0 w-0 h-0 pointer-events-none z-[9999]" style={{ willChange: "transform" }}>
      <div ref={dotRef} className="absolute top-1/2 left-1/2 w-1 h-1 bg-black dark:bg-white rounded-full -translate-x-1/2 -translate-y-1/2" style={{ willChange: "transform" }} />
      {[
        "border-r-0 border-b-0 -translate-x-[150%] -translate-y-[150%]",
        "border-l-0 border-b-0 translate-x-1/2 -translate-y-[150%]",
        "border-l-0 border-t-0 translate-x-1/2 translate-y-1/2",
        "border-r-0 border-t-0 -translate-x-[150%] translate-y-1/2",
      ].map((cls, i) => (
        <div key={i} className={`target-cursor-corner absolute top-1/2 left-1/2 w-3 h-3 border-[3px] border-black dark:border-white ${cls}`} style={{ willChange: "transform" }} />
      ))}
    </div>
  );
}
