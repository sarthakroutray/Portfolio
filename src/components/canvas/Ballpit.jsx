...
class W {
  ...
  update(e) {
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      // Stop micro-jitter by zeroing very small velocities
      if (B.length() < 0.001) {
        B.set(0, 0, 0);
      }
    ...
    for (let idx = r; idx < t.count; idx++) {
      ...
      for (let jdx = idx + 1; jdx < t.count; jdx++) {
        ...
        if (dist < sumRadius && dist > 0.01) {
          const overlap = sumRadius - dist;
          j.copy(_)
            .normalize()
            .multiplyScalar(0.01 * overlap); // Further reduced for smoother merging
          
          // Very soft velocity response
          H.copy(j).multiplyScalar(B.length() * 0.2); 
          T.copy(j).multiplyScalar(N.length() * 0.2);
      ...
      if (t.controlSphere0) {
        ...
        if (dist < sumRadius0) {
          ...
          j.copy(_.normalize()).multiplyScalar(diff * 0.1); // Softer cursor interaction
          H.copy(j).multiplyScalar(B.length());
          I.sub(j);
          B.sub(H);
        }
      }
